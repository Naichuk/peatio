...
describe API::V2::Admin::Adjustments, type: :request do
  ...
  describe 'GET /api/v2/admin/adjustments' do
    ...
    let!(:accepted) { create(:adjustment, receiving_account_number: "BTC-202-#{member.uid}").tap { |a| a.accept!(validator: member) } }
    let!(:rejected) { create(:adjustment, receiving_account_number: "BTC-202-#{member.uid}").tap { |a| a.reject!(validator: member) } }
    ...
    context 'with filters' do
      let!(:adjustment_with_category) { create(:adjustment, category: 'balance_anomaly') }
  ...
  describe 'POST /api/v2/admin/adjustments/action (accept)' do
    let!(:adjustment) { create(:adjustment, receiving_account_number: "btc-202-#{member.uid}") }
    ...
    context 'already accepted' do
      let!(:adjustment) { create(:adjustment, receiving_account_number: "btc-202-#{member.uid}").tap { |a| a.accept!(validator: member) } }
    ...
    context 'already rejected' do
      let!(:adjustment) { create(:adjustment, receiving_account_number: "btc-202-#{member.uid}").tap { |a| a.reject!(validator: member) } }
